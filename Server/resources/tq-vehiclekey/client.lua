...
AddEventHandler("tq-vehiclekey:client:arabayikilitlememlazim", function(plaka)
	...
	vehicles = ESX.Game.GetVehiclesInArea(pCoords, 20)
		if #vehicles == 0 then
			exports['mythic_notify']:SendAlert('error', 'Yakınında hiç araç yok!', 4000)
			--exports['swt_notifications']:Negative("Araç","Yakınlarda Hiç Araç Yok!","top",1500,true)
				...
				local plate = ESX.Math.Trim(GetVehicleNumberPlateText(vehicles[i]))
					if plaka == plate then
						...
						if lock == 1 or lock == 0 then
							...
							exports['mythic_notify']:SendAlert('inform', 'Araç kilitlendi!', 4000)
							--exports['swt_notifications']:Icon("Araç Kilitlendi","top",2500,"grey-9","white",true,"mdi-car")
						...
						elseif lock == 2 then
							...
							exports['mythic_notify']:SendAlert('inform', 'Araç açıldı!', 4000)
							--exports['swt_notifications']:Icon("Araç Açıldı","top",2500,"grey-9","white",true,"mdi-car")
							if not IsPedInAnyVehicle(PlayerPedId(), true) then
								TaskPlayAnim(PlayerPedId(), animdict, "exit", 8.0, 8.0, -1, 48, 1, false, false, false)
							end
							SetVehicleLights(vehicles[i], 2)
						...
						exports['mythic_notify']:SendAlert('error', 'Geçersiz anahtar', 4000)
						--exports['swt_notifications']:Negative("Araç","Geçersiz Anahtar","top",1500,true)
					end
				end
			end
		end
