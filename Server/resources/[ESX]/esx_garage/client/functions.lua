...
SpawnLocalVehicle = function(data)
	...
	if not esx.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
		exports['mythic_notify']:SendAlert('error', 'Arabanı çıkartamazsın, yer dolu!', 4000)  --TriggerEvent('notification', 'Please move the vehicle off the road', 1)
...
SpawnVehicle = function(data,recuperar)
	...
	if not esx.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
		exports['mythic_notify']:SendAlert('error', 'Arabanı çıkartamazsın, yer dolu!', 4000) --TriggerEvent('notification', 'Please move the vehicle off the road', 2)
    ...
    if DoesEntityExist(vehicle) then
		...
		esx.TriggerServerCallback("erp_garage:validateVehicle", function(valid)
			if valid then
                ...
                CloseMenu()
                exports['mythic_notify']:SendAlert('error', 'Araba senin değil!', 4000) --TriggerEvent('notification', 'This vehicle does not belong to you', 2)
			end
...
SaveInGarage = function(garage)
    ...
    exports['mythic_notify']:SendAlert('inform', 'Arabanı ' ..garage.. ' garajına kaydettin!', 4000) --TriggerEvent('notification', 'You saved your vehicle in garage '..garage, 3)
    Citizen.Wait(500)
...
WaitForModel = function(model)
    ...
    if not IsModelValid(model) then
        return exports['mythic_notify']:SendAlert('error', "Bu araç modeli sunucuda bulunmuyor, admin'lerle iletişime geç!", 4000) --TriggerEvent('notification', 'This model does not exist in the game, send a report!', 2)
    end
