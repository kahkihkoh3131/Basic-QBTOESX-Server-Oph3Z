...
esx.RegisterServerCallback('erp_garage:checkMoney', function(source, cb)
	...
	if 200 <=  xPlayer.getQuantity('cash', 200) then
		cb(true)
	else
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Üzerinde yeteri kadar para yok. Gereken: 200$', 8000})
		cb(false)
	end
end)
...
AddEventHandler('erp_garage:pay', function()
	local xPlayer = esx.GetPlayerFromId(source)
	xPlayer.removeInventoryItem('cash', 200)
	TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = '200$ ödedin ve arabayı geri aldın, bundan sonra arabana dikkat et.', 8000})
end)
