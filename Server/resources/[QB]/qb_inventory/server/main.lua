	...
	local PlayerAmmo = {}
		if Player ~= nil then
			if name ~= nil and id ~= nil then
				...
				TriggerClientEvent("inventory:client:OpenInventory", src, PlayerAmmo, Player.inventory, secondInv)
			else
...
function GetStashItems(stashId)
	local Player = ESX.GetPlayerFromId(source)
	local items = {}
	local result = exports.ghmattimysql:executeSync("SELECT * FROM `stashitemsnew` WHERE `stash` = '"..stashId.."'")
				if result[1] ~= nil then 
...
ESX.RegisterServerCallback('qb_inventory:server:GetStashItems', function(source, cb, stashId)
...
AddEventHandler('qb-inventory:server:SaveStashItems', function(stashId, items)
	local result = exports.ghmattimysql:executeSync("SELECT * FROM stashitemsnew WHERE stash = '"..stashId.."'")
	if result[1] ~= nil then
		exports.ghmattimysql:execute("UPDATE `stashitemsnew` SET `items` = '"..json.encode(items).."' WHERE `stash` = '"..stashId.."'")
	else
		exports.ghmattimysql:execute("INSERT INTO `stashitemsnew` (`stash`, `items`) VALUES ('"..stashId.."', '"..json.encode(items).."')")
	end
end)
...
--[[RegisterServerEvent('qb_inventory:server:SaveStashItems')
AddEventHandler('qb_inventory:server:SaveStashItems', function(stashId, items)
	local result = MySQL.Sync.fetchAll("SELECT * FROM stashitemsnew WHERE stash = '"..stashId.."'")
	if result[1] ~= nil then
		MySQL.Async.execute("UPDATE `stashitemsnew` SET `items` = '"..json.encode(items).."' WHERE `stash` = '"..stashId.."'")
	else
		MySQL.Async.execute("INSERT INTO `stashitemsnew` (`stash`, `items`) VALUES ('"..stashId.."', '"..json.encode(items).."')")
	end
end)
...
function GetOwnedVehicleItems(plate)
	local Player = ESX.GetPlayerFromId(source)
	local items = {}
	local result = exports.ghmattimysql:executeSync("SELECT * FROM `trunkitemsnew` WHERE `plate` = '"..plate.."'")
		if result[1] ~= nil then
			if result[1].items ~= nil then
				result[1].items = json.decode(result[1].items)
				if result[1].items ~= nil then 
...
function SaveOwnedVehicleItems(plate, items)
	if Trunks[plate].label ~= "Trunk-None" then
		if items ~= nil then
			...
			local result = exports.ghmattimysql:executeSync("SELECT * FROM trunkitemsnew WHERE plate = '"..plate.."'")
				if result[1] ~= nil then
					exports.ghmattimysql:execute("UPDATE `trunkitemsnew` SET `items` = '"..json.encode(items).."' WHERE `plate` = '"..plate.."'", function(result) 
						Trunks[plate].isOpen = false
					end)
				else
					exports.ghmattimysql:execute("INSERT INTO `trunkitemsnew` (`plate`, `items`) VALUES ('"..plate.."', '"..json.encode(items).."')", function(result) 
						Trunks[plate].isOpen = false
					end)
				end
		end
...
--[[function GetOwnedVehicleItems(plate)
	local Player = ESX.GetPlayerFromId(source)
	local items = {}
	--MySQL.Async.fetchAll("SELECT * FROM `trunkitemsnew` WHERE `plate` = '"..plate.."'", function(result)
		local result = MySQL.Sync.fetchAll("SELECT * FROM trunkitemsnew WHERE plate = '"..plate.."'")
				if result[1] ~= nil then
					if result[1].items ~= nil then
						result[1].items = json.decode(result[1].items)
						if result[1].items ~= nil then 
...
function SaveOwnedVehicleItems(plate, items)
    if Trunks[plate].label ~= "Trunk-None" then
        if items ~= nil then
            ...
            local result = MySQL.Sync.fetchAll("SELECT * FROM `trunkitemsnew` WHERE `plate` = '"..plate.."'")
                if result[1] ~= nil then
                    MySQL.Async.execute('UPDATE trunkitemsnew SET `items` = @items WHERE plate = @plate', {
                    ...
                    MySQL.Async.execute('INSERT INTO trunkitemsnew (items, plate) VALUES (@items, @plate)', {
                        ['@items'] = json.encode(items),
                        ['@plate'] = plate,
                    }, function (rowsChanged)
                        Trunks[plate].isOpen = false
...
function GetOwnedVehicleGloveboxItems(plate)
	local Player = ESX.GetPlayerFromId(source)
	local items = {}
	local result = exports.ghmattimysql:executeSync("SELECT * FROM `gloveboxitemsnew` WHERE `plate` = '"..plate.."'")
	if result[1] ~= nil then 
		if result[1].items ~= nil then
			result[1].items = json.decode(result[1].items)
			if result[1].items ~= nil then 
...
function SaveOwnedGloveboxItems(plate, items)
	if Gloveboxes[plate].label ~= "Glovebox-None" then
		if items ~= nil then
			...
			--exports.ghmattimysql:execute("SELECT * FROM `gloveboxitemsnew` WHERE `plate` = '"..plate.."'", function(result)
				local result = exports.ghmattimysql:executeSync("SELECT * FROM gloveboxitemsnew WHERE plate = '"..plate.."'")
				if result[1] ~= nil then
					exports.ghmattimysql:execute("UPDATE `gloveboxitemsnew` SET `items` = '"..json.encode(items).."' WHERE `plate` = '"..plate.."'", function(result) 
						Gloveboxes[plate].isOpen = false
					end)
				else
					exports.ghmattimysql:execute("INSERT INTO `gloveboxitemsnew` (`plate`, `items`) VALUES ('"..plate.."', '"..json.encode(items).."')", function(result) 
						Gloveboxes[plate].isOpen = false
					end)
				end
		end
...
--[[function GetOwnedVehicleGloveboxItems(plate)
	local Player = ESX.GetPlayerFromId(source)
	local items = {}
	--MySQL.Async.fetchAll("SELECT * FROM `gloveboxitemsnew` WHERE `plate` = '"..plate.."'", function(result)
		local result = MySQL.Sync.fetchAll("SELECT * FROM `gloveboxitemsnew` WHERE `plate` = '"..plate.."'")
				if result[1] ~= nil then 
					if result[1].items ~= nil then
						result[1].items = json.decode(result[1].items)
						if result[1].items ~= nil then 
	...
	if Gloveboxes[plate].label ~= "Glovebox-None" then
		if items ~= nil then
			...
			--MySQL.Async.fetchAll("SELECT * FROM `gloveboxitemsnew` WHERE `plate` = '"..plate.."'", function(result)
				local result = MySQL.Sync.fetchAll("SELECT * FROM gloveboxitemsnew WHERE plate = '"..plate.."'")
				if result[1] ~= nil then
					MySQL.Async.execute("UPDATE `gloveboxitemsnew` SET `items` = '"..json.encode(items).."' WHERE `plate` = '"..plate.."'", function(result) 
						Gloveboxes[plate].isOpen = false
					end)
				else
					MySQL.Async.execute("INSERT INTO `gloveboxitemsnew` (`plate`, `items`) VALUES ('"..plate.."', '"..json.encode(items).."')", function(result) 
						Gloveboxes[plate].isOpen = false
					end)
				end
		end
