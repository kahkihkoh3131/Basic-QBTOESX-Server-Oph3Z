...
MI.Phone.Functions.SetupApplications = function(data) {
    ...
    $.each(data.applications, function(i, app){
        ...
        $(applicationSlot).css({"background-color":"transparent", "margin": "12px"});
        ...
        if (app.app != 'darkweb') {
            if ((!app.job || app.job === MI.Phone.Data.PlayerJob.name) && !blockedapp) {
                ...
                var icon = `<img src="./img/icons/${app.icon}.png" style="margin-left:-1.5px;width: 48px; height: 48px; padding: 1px;"> <p style="color: white; font-family: SF-UI-Text-Regular; font-size: 12.5px; ">${app.tooltipText}</p>`;
                if (app.app == "meos") {
        ...
        if (CanOpenApp) {
            if (MI.Phone.Data.currentApplication == null) {
                ...
                if (PressedApplication == "settings") {
                    ...
                    $.post('http://qb-phone/GetBankData', JSON.stringify({}), function(data){
                    ...
                    $.post('http://qb-phone/DarkwebList', JSON.stringify({}), function(data){
                        SetupDarkweb(data);
                    });
...
MI.Phone.Functions.LoadPhoneData = function(data) {
    ...
    $.post('http://qb-phone/GetLangData', JSON.stringify({}), function(langs){
...
$(document).ready(function(){
    $( "*" ).on("input", function(){
        $.post('http://qb-phone/disableControls', JSON.stringify({}))
    });
