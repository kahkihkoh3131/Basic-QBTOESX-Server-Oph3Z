...
AddEventHandler('esx_policejob:getStockItem', function(itemName, count)
	...
	local sourceItem = xPlayer.getQuantity(itemName)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_police', function(inventory)
		...
		MySQL.Async.execute('INSERT INTO log_ev (Sender, Type, Amount, Reciever)VALUES (@Sender, @Type, @Amount, @Reciever) ', 
			...
			['@Type']   = itemName,
			['@Amount']    = count,
			['@Reciever']    = 'Polis kasasindan aldi' })
		-- else
...
AddEventHandler('playerDropped', function()
    ...
    if _source ~= nil then
        ...
        if xPlayer ~= nil and xPlayer.job ~= nil and xPlayer.job.name == 'police' then
            local currentGPS    = xPlayer.getQuantity('gps')
            Citizen.Wait(5000)
...
--         local xPlayer = ESX.GetPlayerFromId(source)
--         local items = xPlayer.getQuantity(item)
--         if items == nil then
--             cb(0)
--         else
--             cb(items.count)
