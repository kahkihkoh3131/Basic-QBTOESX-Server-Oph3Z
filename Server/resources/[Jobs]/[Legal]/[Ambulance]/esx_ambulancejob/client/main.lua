	...
	if firstSpawn then
		...
		if Config.AntiCombatLog then
			...
			ESX.TriggerServerCallback('esx_ambulancejob:getDeathStatus', function(shouldDie)
				if shouldDie then
					TriggerEvent('notification', _U('combatlog_message'), 2)
...
Citizen.CreateThread(function()
	for k,v in pairs(Config.Hospitals) do
		...
		AddTextComponentSubstringPlayerName('Hastane')
		...
		if itemName == 'medikit' then
			exports['mythic_progbar']:Progress({
				...
				if not cancelled then
					...
					TriggerEvent('notification', _U('used_medikit'), 1)
		...
		elseif itemName == 'bandage' then
			exports['mythic_progbar']:Progress({
				...
				if not cancelled then
					...
					TriggerEvent('notification', _U('used_bandage'), 1)
					TriggerServerEvent('esx_ambulancejob:removeItem','bandage')
				end
			end)
		end
	else
		TriggerEvent('notification', 'Yerdeyken bunu kullanamazsın!', 2)
	end
end)
...
function StartDeathTimer()
	...
	Citizen.CreateThread(function()
		...
		while earlySpawnTimer > 0 and isDead do
			Citizen.Wait(0)
			text = "YENIDEN DOGMANA ~r~" .. earlySpawnTimer .. "~w~ SANIYE VAR"
