							...
							if data2.current.stored then
								...
								if foundSpawn then
									...
									ESX.Game.SpawnVehicle(data2.current.model, spawnPoint.coords, spawnPoint.heading, function(vehicle)
										...
										TriggerEvent('notification', _U('garage_released'), 1)
									end)
								end
							else
								TriggerEvent('notification', _U('garage_notavailable'), 2)
							end
						end, function(data2, menu2)
							menu2.close()
						end)
					else
						exports['mythic_notify']:SendAlert('error', 'Garaj boş', 4000)
					end
				else
					exports['mythic_notify']:SendAlert('error', 'Garaj boş', 4000)
				end
...
function StoreNearbyVehicle(playerCoords)
	...
	if #vehicles > 0 then
		...
		TriggerEvent('notification', _U('garage_store_nearby'), 2)
		return
	end
	...
	ESX.TriggerServerCallback('esx_ambulancejob:storeNearbyVehicle', function(storeSuccess, foundNum)
		if storeSuccess then
			...
			isBusy = false
			exports['mythic_notify']:SendAlert('error', 'Aracını garaja koydun', 4000)
		else
			exports['mythic_notify']:SendAlert('error', 'Aracını garaja koyamadınız', 4000)
		end
	end, vehiclePlates)
end
...
function GetAvailableVehicleSpawnPoint(hospital, part, partNum)
	...
	if found then
		...
		TriggerEvent('notification', _U('garage_blocked'), 2)
		return false
	end
end
