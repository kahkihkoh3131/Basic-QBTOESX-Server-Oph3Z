		...
		if xTarget then
			if deadPlayers[playerId] then
				if Config.ReviveReward > 0 then
					...
					TriggerClientEvent('inventory:client:ItemBox',src,ESX.GetItems()['cash'],'add',Config.ReviveReward)
...
AddEventHandler('esx_ambulancejob:itemnotif', function(item,label ,count)
	local xPlayer = ESX.GetPlayerFromId(source)
	local esya = xPlayer.getQuantity(item)
...
AddEventHandler('esx_ambulancejob:polisitemver', function(item, miktar)
	...
	TriggerClientEvent('inventory:client:ItemBox',src,ESX.GetItems()[item],'add',miktar)
end)
...
ESX.RegisterServerCallback('esx_ambulancejob:removeItemsAfterRPDeath', function(source, cb)
	...
	if Config.RemoveCashAfterRPDeath then
		if xPlayer.getMoney() > 0 then
			...
			--dclog(xPlayer, '$ '..hastanedeger.. ' parasını hastanede kaybetti')
		end
	...
	if Config.RemoveItemsAfterRPDeath then
		for i=1, #xPlayer.inventory, 1 do
			-- if xPlayer.inventory[i].count > 0 then
				xPlayer.setInventoryItem(xPlayer.inventory[i], 0)
			-- end
		end
	end
	...
	if price == 0 then
		...
		if price <= xPlayer.getQuantity('cash', price) then
			...
			MySQL.Async.execute('INSERT INTO owned_vehicles (owner, vehicle, plate, type, job, `stored`) VALUES (@owner, @vehicle, @plate, @type, @job, @stored)', {
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'EMS aracını ' ..price.. '$ aldın', 8000})
				cb(true)
			end)
		else
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Yeteri kadar paran yok. Gereken: ' ..price.. '$', 8000})
			cb(false)
		end
	end
end)
...
AddEventHandler('esx_ambulancejob:removeItem', function(item)
	...
	TriggerClientEvent('inventory:client:ItemBox',src,ESX.GetItems()[item],'remove',1)
...
AddEventHandler('esx_ambulancejob:giveItem', function(itemName, amount)
	...
	-- if xPlayer.canCarryItem(itemName, amount) then
		...
		TriggerClientEvent('inventory:client:ItemBox',src,ESX.GetItems()[itemName],'add',amount)
	-- else
...
ESX.RegisterServerCallback('esx_ambulancejob:getDeathStatus', function(source, cb)
	...
	MySQL.Async.fetchScalar('SELECT is_dead FROM users WHERE identifier = @identifier', {
		...
		if isDead then
			...
			--dclog(xPlayer, 'Combat log uyarısı!!!!!!!!!!!!!!!!!!!!!!')
		end
...
function dclog(xPlayer, text)
    ...
    local data = {
      ["username"] = 'Combat Log - Hastane Log',
      ["avatar_url"] = 'https://cdn.discordapp.com/attachments/779842441875816479/783016696268455966/prplogo.png',
      ["embeds"] = {{
        ["author"] = {
