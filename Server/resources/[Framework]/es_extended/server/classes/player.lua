function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, loadout, name, coords)
	...
	self.addInventoryItem = function(item, amount, slot, info)
		...
		if itemInfo == nil then TriggerClientEvent('okokNotify:Alert',self.source, "MTH - Envanter", "Üzerinize almaya çalıştığınız item bulunamadı. Sunucu Sahibi ile iletişime geçin", 2000, 'error')
		...
		if (totalWeight + (itemInfo.weight * amount)) <= self.maxWeight then
			...
			TriggerClientEvent('okokNotify:Alert',self.source, "MTH - Envanter", "Maksimum Ağırlığa ulaştınız!"..self.maxWeight..' kg', 5000, 'error')
